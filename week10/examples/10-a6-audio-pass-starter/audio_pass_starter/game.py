...
class TitleScene:
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        if event.type == pygame.KEYDOWN:
            ...
            # Added increase and decrease volume actions here 
            # to prevent held key and volume increase/decrease suddenly
            # and drastically
            if event.key == pygame.K_F1:
                self.game.audio._adjust_volumes(True)
            if event.key == pygame.K_F2:
                self.game.audio._adjust_volumes(False)
    ...
    def draw(self) -> None:
        ...
        # Added instruction and display on how to increase or decrease loop channel volume.
        volume = self.game.font(22).render("Press F1 to increase volume | Press F2 to decrease volume", True, SUBTLE)
        ...
        screen.blit(volume, volume.get_rect(center=(self.game.SCREEN_W // 2, 432)))
...
class EndScene:
    ...
    def on_enter(self) -> None:
        # Added win and lose sounds so if player enters EndScene depending on if they won or not, 
        # a meaningful event sound will be played portraying if player won or not and added shutdown
        # to prevent ingame sound to transfer to end scene
        self.game.audio.shutdown()
        if self.won:
            self.game.audio.play("win_loop")
        else:
            self.game.audio.play("lose_loop")
